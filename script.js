function verificar() {
    ...
    if (v.value.length == 0 || s.value.length == 0 || af.value.length == 0) {
        ...
        res.innerHTML = `<p>Para pagar uma casa de R$${vc} em ${anofin} anos a prestação será de ${vp} <br></p>`

        res.innerHTML += `Empréstimo Negado!`
    } else if (sal >= vp) {
        res.innerHTML = `<p>Para pagar uma casa de ${vc} em ${anofin} anos,a prestação será de ${vp}.<br></p> `
